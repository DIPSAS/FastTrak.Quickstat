...
| k | **Export is refused when the matrix is empty or unlocked, *and* both menu items are greyed until it is neither** | The Delphi writes `(not ready)` into every cell, or a phantom `"nil"` row, and both are reachable: *Save dataset to CSV* is always enabled and *Open in Excel* latches on. The guard was the whole of this row until the parity pass reported the greying as well (Phase 5); `DatasetViewModel.CanExport` is now the enable rule for both commands and the guard is the execute-time half of the same question | delete `DatasetViewModel.CanExport` and `EnsureExportable` |
...
| o | **The `Unique name` box stops at 80 characters** | `Report.QuickStat.Title` is `varchar(80)` and `Report.AddQuickStat` upserts on `(StudyId, Title)`, so an over-long title does not merely lose its tail — it overwrites whichever package shares its first 80, and nothing anywhere raises: assigning to a `VARCHAR(80)` *parameter* truncates in silence where an `INSERT` would fail. `edtTitle` has no `MaxLength`, so this is a divergence rather than parity; taken by the product owner on 2026-09-01 (PORT-PLAN.md §8.11 (13)). `Comments` stays uncapped — that column is `varchar(MAX)` | one attribute and `PackagedSelection.MaxTitleLength` |
| p | **The modals put `OK` first and `Cancel` second** | Both Emetra dialogs place OK on the right (`btnSave.Left` 280 against `btnClose.Left` 184), which is the opposite of the Windows convention and of `NotificationDialog` in this same application, whose `Yes` has always come first — so the port disagreed with itself as well as with the platform. Switched by the product owner on 2026-09-01 (PORT-PLAN.md §7.3). Sizes and the 4 + 4 + 16 spacing are still §E's | swap two `<Button>` elements and their margins in each of the two dialogs, and flip the two `TheButtonBarPutsOkFirst` assertions |
