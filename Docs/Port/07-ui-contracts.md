...
| n | **The version string is whatever the assembly reports** | Nothing sets a `Version`, so it reads `1.0.0.0` while the shipped Delphi build says `22.12.21.547`. Inventing one would put a false version in front of users and in every log line | a packaging decision; set `<Version>` in the build |
