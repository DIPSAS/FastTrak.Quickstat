...
- **Under Continua** it is bound to the `$Source.FastTrakDevelop` source. *What branch that source
  ...
  inference is read off the name. If `FastTrakDevelop` happened to track a tarmscreening ref, the
  build would succeed. Treat it as probable, not established — and note that it **stays**
  unobserved on purpose: R13 was closed on 2026-09-03 with this half unanswered, because nothing
  downstream depends on which way it falls. This bullet records an inference, not a gap somebody is
  expected to fill.
...
8. A human parity pass against `05-ui-spec.md` finds no unexplained differences.
   ...
   **Written out as `Docs/Port/08-parity-checklist.md`**, so the pass is a walk rather than a
   re-read of 1 163 lines of specification. **47 items** still need eyes; everything already pinned
